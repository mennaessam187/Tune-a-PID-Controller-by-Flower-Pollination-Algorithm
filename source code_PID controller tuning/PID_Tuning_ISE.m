...
% Simulation time
t_sim = 20;
tsim = 0:0.1:t_sim;     % Time vector
u = ones(size(tsim));   % Constant input (step)
ns = [1 2];
ds = [1 2 3 5];

G = tf(ns, ds);
% Simulate closed-loop system without PID
G_uncontrolled = feedback(G, 1);
y_uncontrolled = lsim(G_uncontrolled, u, tsim);
% Plot closed-loop response without controller
figure(1);
...
% Evaluate initial objective function (ISE only)
ffb = zeros(1, N);
for i = 1:N
